...
{
    $deleteimage = $row['profilePicFile'];

}
unlink($folder.$deleteimage);
move_uploaded_file($file,$target_file); 
$result=mysqli_query($db,"UPDATE user SET profilePicFile='$image_file',bio='$title' WHERE userID=$id");


	}
	else 
	{
        $res=mysqli_query($db,"SELECT * FROM user WHERE userID=$id LIMIT 1");
